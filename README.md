<h1 align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=32&duration=3000&pause=1000&color=00F7FF&center=true&vCenter=true&width=600&lines=Hi+There%2C+I'm+Jaswanth+A+%F0%9F%91%8B;ECE+Student+%7C+Future+Engineer;Java+%7C+Web+Developer;UI%2FUX+Enthusiast;Video+%26+Photo+Editor;Always+Learning+Something+New+%F0%9F%9A%80" alt="Typing SVG" />
</h1>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=200&section=header&text=Welcome%20To%20My%20Profile!&fontSize=40&fontColor=fff&animation=twinkling" />
</p>

<p align="center">
  <img src="https://user-images.githubusercontent.com/74038190/213910845-af37a709-8995-40d6-be59-724526e3c3d7.gif" width="450">
</p>

---

### 🧑‍💻 About Me

<img align="right" width="260" src="https://user-images.githubusercontent.com/74038190/212284158-e840e285-664b-44d7-b79b-e264b5e54825.gif">

```bash
jaswanth@ece-engineer ~ % neofetch

       ╭──────────────╮
       │   ⚡  JASWANTH │
       ╰──────────────╯

  Degree ........... B.E - Electronics & Communication
  Batch ............ 2024 – 2028
  Learning ......... Java, Web Development
  Exploring ........ UI/UX Design (Figma)
  Creating ......... Video & Photo Editing
  Core Strength ..... Circuits + Code
  Status ........... 🌱 Always Learning
```

<br>

<details>
<summary>⚡ <b>View as Java</b></summary>

```java
public class Jaswanth {
    String   degree   = "B.E - ECE";
    String   batch    = "2024 - 2028";
    String[] learning = { "Java", "Web Dev", "UI/UX" };
    String[] hobbies  = { "Video Editing", "Photo Editing" };
    boolean  curious  = true;

    void main() {
        System.out.println("From circuits to code — building things that work & look good ⚡");
    }
}
```

</details>

<br clear="right"/>

---

### 🚀 Tech Stack & Tools

<p align="center">
  <img src="https://skillicons.dev/icons?i=java,html,css,js,react,figma,photoshop,premiere,git,github,vscode,mysql&theme=dark" />
</p>

---

### 💬 Quote of the Moment

<p align="center">
  <img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight" />
</p>

---

### 📈 Skill Levels

<p align="center">
  <img src="https://img.shields.io/badge/Java-60%25-f89820?style=for-the-badge&logo=openjdk&logoColor=white" /><br><br>
  <img src="https://img.shields.io/badge/HTML%20%2F%20CSS-90%25-e34c26?style=for-the-badge&logo=html5&logoColor=white" /><br><br>
  <img src="https://img.shields.io/badge/JavaScript-60%25-f0db4f?style=for-the-badge&logo=javascript&logoColor=black" /><br><br>
  <img src="https://img.shields.io/badge/UI%2FUX%20(Figma)-70%25-a259ff?style=for-the-badge&logo=figma&logoColor=white" /><br><br>
  <img src="https://img.shields.io/badge/Video%20%2F%20Photo%20Editing-80%25-00c2ff?style=for-the-badge&logo=adobepremierepro&logoColor=white" />
</p>

---

### 🎯 What I'm Focused On

<table align="center">
  <tr>
    <td align="center">☕<br><b>Java</b><br><sub>OOP & Backend Basics</sub></td>
    <td align="center">🌐<br><b>Web Dev</b><br><sub>HTML · CSS · JS</sub></td>
    <td align="center">🎨<br><b>UI/UX</b><br><sub>Figma & Wireframes</sub></td>
    <td align="center">🎬<br><b>Editing</b><br><sub>Video & Photo</sub></td>
    <td align="center">⚡<br><b>ECE Core</b><br><sub>Circuits & Communication</sub></td>
  </tr>
</table>

---

### 🏅 Milestones

<p align="center">
  <img src="https://img.shields.io/badge/🎓_Pursuing-B.E._ECE-1f2937?style=for-the-badge" />
  <img src="https://img.shields.io/badge/💻_Learning-Java_&_Web_Dev-1f2937?style=for-the-badge" />
  <img src="https://img.shields.io/badge/🎨_Building-UI%2FUX_Skills-1f2937?style=for-the-badge" />
  <img src="https://img.shields.io/badge/🎬_Creating-Video_&_Photo_Edits-1f2937?style=for-the-badge" />
</p>

---

### 📫 Let's Connect

<p align="center">
  <a href="https://linkedin.com/in/YOUR_LINKEDIN" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:YOUR_EMAIL@example.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://instagram.com/YOUR_INSTAGRAM" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" />
  </a>
</p>

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=120&section=footer" />
</p>

<h3 align="center">✨ "Design it. Code it. Ship it." ✨</h3>
